...
fun checkAdjacent(list: List<String>, visitedPoints: MutableList<Point>, index1: Int, index2: Int, steps: Int): Int {
    ...
    if ((index1 > 0) && list[index1 - 1][index2] != '9' && !visitedPoints.contains(Point(index1 - 1, index2))) {
        result += checkAdjacent(list, visitedPoints, index1 - 1, index2, steps)
    }

    if ((index1 < list.size - 1) && list[index1 + 1][index2] != '9' && !visitedPoints.contains(Point(index1 + 1, index2))) {
        result += checkAdjacent(list, visitedPoints, index1 + 1, index2, steps)
    }

    if ((index2 > 0) && list[index1][index2 - 1] != '9' && !visitedPoints.contains(Point(index1,index2 - 1))) {
        result += checkAdjacent(list, visitedPoints, index1, index2 - 1, steps)
    }

    if ((index2 < list[index1].length - 1) && list[index1][index2 + 1] != '9' && !visitedPoints.contains(Point(index1,index2 + 1))){
        result += checkAdjacent(list, visitedPoints, index1, index2 + 1, steps)
    }
